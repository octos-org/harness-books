...
### 1.6 模型时空失效的机理：自回归是一个有限预算的时空滤波器

§1.4–1.5 已经从现象层说清楚一件事：有效工作区间小于容量上限。但如果停在现象，读者很容易把它当成“模型还不够好、以后会修好”的暂时缺陷。本节要把话说透：**有效工作区间的存在，是 Transformer 加自回归这套架构在有限算力与显存预算下的数学必然，而不是工程缺陷。** 把机理讲透很重要，因为后文每一项 harness 能力，都是对其中某一条失效机理的对症补偿；不先讲清楚模型天然缺什么，harness 就会被误读成一袋经验技巧。

一个统一的视角能把前面几个类比收束起来：**自回归加 attention，本质上就是一个有限预算下的时空滤波器。** 它对“位置远”（空间）和“时间旧”（时间）的信号都会衰减。§1.1.7 的 cache 局部性、§1.1.8 的放大器线性区，其实是同一件事在不同介质里的两个影子——滤波器的频率响应在远端衰减，正是它们共同的底层。

#### 1.6.1 空间轴：位置越远，有效敏感度越低
...
第三，`有限残差维度等于有限信道容量`。残差流是固定宽度的向量，却要承载整段上下文里的相关信息。由信息论，一个定宽状态无法无损保存无界上下文——越旧、越远的细节越会被压缩、覆盖。这正是 §1.1.1 “LLM 看起来记得、却没有持久事实”在数值层面的根。
...
把这四条叠起来，就得到 §1.5 那条经验现象的机理解释：**上下文窗口不是一块平坦内存，它的“有效分辨率”从光标附近向远处递减。** *Lost in the Middle* 的 U 形偏置、NoLiMa 在 32K 上把 GPT-4o 从 99.3% 打到 69.7%，都是这条递减曲线在不同任务上的投影。

而工业界已经把这条“非均匀分辨率”直接做进了架构。为了在百万 token 上还付得起算力，新一代模型不再对全程做均匀的全注意力：滑窗注意力是字面上的矩形窗，窗外直接置零；线性注意力与 SSM（如 MiniMax 的做法）是字面上的指数衰减核；KV 压缩（DeepSeek V4 的 CSA 与 HCA、Kimi 的 MLA）则是有损降采样——近端最近的 token 保留高分辨率、不压缩，越往远端、KV 被越激进地聚合：CSA 把每 m 个 token 压成一条，HCA 把每 m′（远大于 m）个 token 压成一条。换句话说，**所谓“1M context”不是一条处处等价的传送带，而是一组分辨率非均匀的滤波器：近端细、远端粗。** 这恰好把 §1.4 “容量上限不等于有效工作区间”从经验建议升级成了架构事实——连模型厂商自己，都在用压缩换取这个容量上限。[^nonuniform-kv-ch1]
...
#### 1.6.2 时间轴：时间越旧，有效敏感度越低

长任务的失效看起来是“时间”问题——跑得越久越偏——但要拆成两半才讲得透：一半可约为空间问题，另一半不可约。

`记忆老化可约为空间衰减`。vanilla Transformer 除 KV cache 外没有跨步状态——“k 步前还记不记得”这件事，等价于“那个 token 还在不在窗口里、当前 token 还 attend 得到它吗”。随着生成推进，一个旧事实与当前光标的相对位置距离单调增大，于是按 §1.6.1 的四条空间机理被衰减，最终被窗口、压缩或逐出淘汰。所以“随时间遗忘”在机理上就是“随相对距离衰减”——时间问题塌缩成了空间问题。这正是 §1.1.1 “看起来记得、却没有持久事实”的来源。
...
用 §1.1.8 的电路语言收口最干净：一个递归（IIR）系统有两种相互独立的退化——`冲激响应衰减`（远端输入被衰减，等于记忆与空间衰减，可约）和`回路噪声累积`（把自己带噪的输出回灌，等于误差复合，不可约）。这正好对应 §1.1.8 早就给出的两类器件：`正反馈锁存器`把关键事实锁住、对抗记忆衰减，`负反馈环`把错误送回纠偏、对抗误差复合。本节只是把“为什么需要这两类器件”从类比落到了机理。

这里给出一个后文会反复使用的可操作判据：**把丢失的信息重新塞回光标附近——如果失效被修复，它是空间问题，可由上下文工程补偿；如果模型本来就拿到了信息却仍然漂移、越错越多，它是不可约的时间核，只能由验证、缩短 horizon 和人在环上来兜住。**

### 1.7 机理对应能力：本书后续章节的提纲

把上面六条机理排开，本书后续的每一项 harness 能力，都可以被读成“对某一条失效机理的对症补偿”。下面这张表就是全书的提纲：左边是模型层“天然缺什么”，右边是 harness 层“补什么、在哪一章展开”。

| 失效机理（模型层） | 失效表现 | 对症的 harness 能力 | 展开章节 |
|---|---|---|---|
| softmax 稀释 | 上下文越长，关键证据越被“平均掉” | 认知层级缓存控制：admission / eviction、compaction、检索式“短密近”证据切片 | §1.1.7、第 4 章（事实流）、第 9 章 |
| RoPE 距离衰减 / 外推退相干 | 远距离关联弱、超长外推失效 | 上下文分层加按需调入（L1 prompt / L2 `AGENTS.md`·skills·memory / L3 事实流）；把关键事实重新拉回光标附近 | §1.1.7、第 4 章、第 6 章 |
...
| 非均匀 KV 分辨率 | 同一窗口不同位置可用性不均（U 形） | 把容量上限与有效工作区间分开设计；短密近证据加检索加分层缓存，不默认塞满 | §1.4–1.5、第 4 章 |
...
第一，**harness 不是一袋经验技巧，而是一组针对已知失效机理的控制器件。** 这正是把 §1.1.6 “harness 是外层反馈控制器”落到实处的方式：`AGENTS.md`、skills、memory、hooks、validators、dashboard、swarm 协作，分别是这套反馈控制器上的不同传感器、执行器和约束器件，而每一个都对准一条具体的失效机理。

第二，**空间类机理大多能被上下文工程“补偿”，但时间轴上那条“不可约误差复合”补偿不了、只能被“兜住”。** 这条边界不是修辞，而是工程事实：它正是 §1.2–1.3 反复出现的“上下文工程必要但不充分、必须上升到 harness 工程”的根本原因，也是为什么第 8 章要用群体编排去缩短单链 horizon、第 9 章要把系统拆成短回路与长回路。把更大的窗口当成解药，只能缓解空间类机理；时间类的不可约核，必须靠运行时的验证与人的监督来关闭。
