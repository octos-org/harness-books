...
如果第 7 章守的是能力入口，第 8 章守的就是用户表面的真实性。对 harness 来说，前端不是装饰层，而是运行时事实能否被用户正确看见的最后一跳。只要这一跳开始自作主张，系统很快就会掉回第 5.1 到第 5.3 节那些事故里。
...
[^replay-truth-ch6]: 本章在此处综合 Codex `app-server` 中 thread / turn / item 的统一事件流设计，以及 Claude Code transcript、resume 与 agent summary 机制，说明 UI replay 必须建立在统一的后端事实模型之上；对应第 21 章参考文献 21、24。

---

## 9. 操作员控制面与发布真实性

到第 8 章为止，我们回答的是“用户如何看到真相”。第 9 章回答的是另一半问题：“操作员如何足够快地理解真相，并用它做发布决策”。这一步非常关键，因为很多团队到了这里会犯一个常见错误：系统内部已经有不少事件和日志，于是误以为“可观测性差不多有了”。但对 harness 来说，真正需要的是控制面，不是日志堆。
...
只看计数器是答不出这些问题的。计数器告诉你系统“最近很忙”或“失败率升高了”，但答不出“这次具体为什么错”。这就是第 5.6 节会出现操作员盲区的原因。
...
这样做的目的，不是让 dashboard 更华丽，而是让它与第 6 章那张运行时总图一一对齐。只要 dashboard 上存在“只在图表里出现、但在事实流里找不到来源”的状态，系统就又多了一条影子真相。

### 9.4 小型 fleet 不是测试补充，而是发布事实
...
第 5.7 节讲“失败模式博物馆”时，重点是归档。到了第 9 章，重点则变成回流：每一个高价值失败模式，最终都应该映射成 dashboard 信号、门禁脚本或 operator runbook。只有这样，事故故事才会变成控制面的免疫力，而不是写完复盘就结束。

[^operator-plane-ch7]: 本章在此处综合 Claude Code `agentSummary.ts`、`diskOutput.ts`、resume 相关实现，以及 Codex `Thread` / `Turn` / `Item` 的对象分层，用来说明操作员控制面应如何建立在统一事实流之上；对应第 21 章参考文献 21、24。
