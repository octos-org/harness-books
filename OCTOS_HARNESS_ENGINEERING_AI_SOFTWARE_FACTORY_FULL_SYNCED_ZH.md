...
到这里，第 1 章只做了一件事：说明 harness 不是可选的产品装饰，而是让 LLM 长任务留在有效工作区间内的控制系统。接下来的第 2 到第 4 章会依次回答三个问题：成熟团队是如何跨过这道鸿沟的；Octos 现在具体卡在哪里；这些问题最后应当落成什么样的运行时架构。

[^layer1-harness-ch1]: 本章在此处综合 Fowler 关于 coding agent harness 的框架，以及 OpenAI 关于 1M LOC 与 `AGENTS.md + docs` 的工程实践；对应第 19 章参考文献 1、2。
[^layer2-harness-ch1]: 本章在此处综合 Anthropic 关于 managed agents 与 long-running harness 的接口抽象，以及 Milvus execution layer、Neo4j memory/context layer 的观点；对应第 19 章参考文献 3、4、6、7。

---

## 2. 成熟度鸿沟：灵光一闪 vs 工厂化输出

上一章回答的是“为什么必须有 harness”；这一章回答的是“成熟团队究竟在建设什么，才能从 demo 式聪明跨到工厂化输出”。先看 OpenAI 这类外部案例，不是为了抄作业，而是为了把成熟度鸿沟说具体。
...
下一章会把这个鸿沟落回 Octos 自己的失败现场。也就是说，我们不再停留在“成熟团队长什么样”，而是直接回答“Octos 离它还差什么”。
...
[^agents-catalog-ch2]: 本章在此处综合 OpenAI 与 Fowler 关于 `AGENTS.md` 作为短目录、细节下沉到 `docs/` 的做法；对应第 19 章参考文献 1、2。
...
如果说第 2 章回答的是工厂化输出长什么样，那么这一章回答的是 Octos 现在离它还差什么。这里不再讨论理念，而是把差距拆成可命名、可归档、可修复的失败类别。
...
当事实流、桥接和 UI 回放都具备之后，系统还需要一个操作员视角来证明这些契约在真实主机、真实 session 和真实重连路径上成立。第 7 章讨论的，就是这种“发布事实”。
...
到这里为止，本书讨论的主要还是单个系统如何说真话。第 8 章再往前走一步：当系统开始扩展为多 agent、多负责人协作时，harness 如何把复杂度吸收掉，而不是放大混乱。
...
前面 1 到 8 章给的是论证、案例和架构部件；这一章把它们压缩成可执行原则和必须禁止的反模式。也可以把它看成前面整条论证链的工程压缩版。
...
边界讲清之后，剩下的工作就是把前面各章的要求压成能执行的检查项。第 12 章不是新观点，而是把整本书收敛成可以拿来合并前、上线前、事故后的动作清单。
...
前面的章节已经给出原则、边界和检查表；这一章补两段最小架构片段，帮助团队把抽象要求落成具体事实流和契约执行流。
