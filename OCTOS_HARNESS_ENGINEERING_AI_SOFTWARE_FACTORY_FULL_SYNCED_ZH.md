...
要理解这个工作区间为什么存在、它的边界从哪里来，必须先看清模型在架构层面天然会怎样失效。所以本章先讲机理（§1.1）与它的实测印证（§1.2），把每条机理对应到后续的 harness 能力（§1.3），再展开“外层控制系统”到底是什么、从哪来（§1.4 起）。换句话说：**先讲模型为什么必然失效，再讲控制系统如何回应。** 这条顺序不是叙事偏好，而是因为 harness 的整个设计，都是从这些失效机理推导出来的。

### 1.1 模型时空失效的机理：自回归是一个有限预算的时空滤波器

本章开头给了两个类比：CPU 缓存的局部性、放大器的线性区。它们看起来分属存储与电路两个世界，其实是同一件事的两个影子。把这件事讲透，就是本节的任务：**有效工作区间的存在，是 Transformer 加自回归这套架构在有限算力与显存预算下的数学必然，而不是“模型还不够好、以后会修好”的暂时缺陷。** 讲透机理很重要，因为后文每一项 harness 能力，都是对其中某一条失效机理的对症补偿；不先讲清楚模型天然缺什么，harness 就会被误读成一袋经验技巧。

把那两个类比收束成一句话：**自回归加 attention，本质上就是一个有限预算下的时空滤波器。** 它对“位置远”（空间）和“时间旧”（时间）的信号都会衰减——cache 局部性说的是空间维度上的工作集，放大器线性区说的是不失真的稳定带宽，而它们各自的电路与历史，会在 §1.4 展开。
...
第三，`有限残差维度等于有限信道容量`。残差流是固定宽度的向量，却要承载整段上下文里的相关信息。由信息论，一个定宽状态无法无损保存无界上下文——越旧、越远的细节越会被压缩、覆盖。这正是 §1.4.1 将展开的“LLM 看起来记得、却没有持久事实”在数值层面的根。
...
`记忆老化可约为空间衰减`。vanilla Transformer 除 KV cache 外没有跨步状态——“k 步前还记不记得”这件事，等价于“那个 token 还在不在窗口里、当前 token 还 attend 得到它吗”。随着生成推进，一个旧事实与当前光标的相对位置距离单调增大，于是按 §1.1.1 的四条空间机理被衰减，最终被窗口、压缩或逐出淘汰。所以“随时间遗忘”在机理上就是“随相对距离衰减”——时间问题塌缩成了空间问题。这正是 §1.4.1 将展开的“看起来记得、却没有持久事实”的来源。
...
用电路语言收口最干净：一个递归（IIR）系统有两种相互独立的退化——`冲激响应衰减`（远端输入被衰减，等于记忆与空间衰减，可约）和`回路噪声累积`（把自己带噪的输出回灌，等于误差复合，不可约）。这正对应两类互补的反馈器件：`正反馈锁存器`把关键事实锁住、对抗记忆衰减，`负反馈环`把错误送回纠偏、对抗误差复合——它们的电路史（Black 的负反馈放大器、SRAM 的双稳态锁存）会在 §1.4.8 展开。本节先把“为什么需要这两类器件”从机理上立住。
...
### 1.3 机理对应能力：本书后续章节的提纲

把 §1.1 的六条机理排开，本书后续的每一项 harness 能力，都可以被读成“对某一条失效机理的对症补偿”。下面这张表就是全书的提纲：左边是模型层“天然缺什么”，右边是 harness 层“补什么、在哪一章展开”。

| 失效机理（模型层） | 失效表现 | 对症的 harness 能力 | 展开章节 |
|---|---|---|---|
| softmax 稀释 | 上下文越长，关键证据越被“平均掉” | 认知层级缓存控制：admission / eviction、compaction、检索式“短密近”证据切片 | §1.4.7、第 4 章（事实流）、第 9 章 |
| RoPE 距离衰减 / 外推退相干 | 远距离关联弱、超长外推失效 | 上下文分层加按需调入（L1 prompt / L2 `AGENTS.md`·skills·memory / L3 事实流）；把关键事实重新拉回光标附近 | §1.4.7、第 4 章、第 6 章 |
...
第一，**harness 不是一袋经验技巧，而是一组针对已知失效机理的控制器件。** 这正是把 §1.4.6 “harness 是外层反馈控制器”落到实处的方式：`AGENTS.md`、skills、memory、hooks、validators、dashboard、swarm 协作，分别是这套反馈控制器上的不同传感器、执行器和约束器件，而每一个都对准一条具体的失效机理。

第二，**空间类机理大多能被上下文工程“补偿”，但时间轴上那条“不可约误差复合”补偿不了、只能被“兜住”。** 这条边界不是修辞，而是工程事实：它正是 §1.5–1.6 将给出的“上下文工程必要但不充分、必须上升到 harness 工程”的根本原因，也是为什么第 8 章要用群体编排去缩短单链 horizon、第 9 章要把系统拆成短回路与长回路。把更大的窗口当成解药，只能缓解空间类机理；时间类的不可约核，必须靠运行时的验证与人的监督来关闭。
...
### 1.4 从 LLM 工作机理到 Agentic AI：为什么必须有外层工作环境

§1.1–1.3 已经说清楚模型天然缺什么、以及每条缺陷对应哪项 harness 能力。接下来的问题是：这套“外层控制系统”到底从哪来、为什么偏偏是它。答案要从更低一层讲起：LLM 是什么样的机器，它天然缺什么；agent 这个概念怎样从 AI 理论里的自治系统，演化成今天围绕 LLM 的工具执行体；如果把 LLM 当成一个需要工作集管理的 cache system，或者一个高增益但会漂移的放大器系统，外层到底要补什么机制。

答案会自然落到 agentic AI。所谓 agentic AI，不是“让模型自己想办法”的浪漫说法，而是给 LLM 搭一个能感知、行动、记忆、验证、恢复和被人类监督的工作环境。这个环境提供的负反馈循环，才是 LLM 能稳定高效工作的前提。

#### 1.4.1 LLM 的能力来自条件生成，不来自内置状态机
...
#### 1.4.3 Agent 概念的演化：从自治系统到 LLM 工具执行体
...
#### 1.4.4 Agentic AI 做的事：给 LLM 建一个稳定工作环境
...
#### 1.4.7 从 delay line 到 cache：为什么系统必须管理工作集
...
#### 1.4.8 从 Harold Black 到半导体反馈：为什么稳定要靠限幅与锁存
...
这两类机制，正是 §1.1.2 在机理层面点到的“正反馈锁存器”与“负反馈环”的电路原型。对 harness 特别有启发，因为一个长期运行的 agent 同样需要这两类机制：
...
#### 1.4.9 回到现代 agent：控制论如何变成 harness engineering
