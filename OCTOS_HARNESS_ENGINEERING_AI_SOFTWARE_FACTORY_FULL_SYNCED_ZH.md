...
把那两个类比收束成一句话：**自回归加 attention，本质上就是一个有限预算下的时空滤波器。** 它对“位置远”（空间）和“时间旧”（时间）的信号都会衰减。（这两个类比各自的电路与历史——维纳、Bell 实验室、cache 演化——留到 §1.3，作为这套控制语言的血统；本节只取它们的直觉。）

#### 1.1.1 空间轴：位置越远，有效敏感度越低

沿上下文方向，至少有四条机理在同时压低远端 token 的有效权重，外加一条“非均匀分辨率”把它做进了今天的架构。

第一，`softmax 的注意力质量守恒导致稀释`。attention 输出是对窗口内所有 token 的凸组合，权重之和恒为 1；窗口里有 n 个 token，平均每个只分到 1/n 的“注意力预算”。要让模型把权重牢牢压在某一个远端关键 token 上，它的 logit 必须显著高过其余，量级大约是 ln n。但 logit 是有界的——RMSNorm、QK-Norm，以及像 Kimi 报告里专门设计的 QK-Clip，都在压制它；不封顶时，attention logit 会冲过 1000 量级、训练直接发散。logit 被一个与 n 无关的常数卡住，单个 token 能拿到的最大权重就随 n 增大趋于零。结论几乎是算术而非调参：**窗口越满，对任意单个远端证据的“最锐检索”越弱。**[^softmax-dilution-ch1][^bounded-logit-ch1]
...
第三，`有限残差维度等于有限信道容量`。残差流是固定宽度的向量，却要承载整段上下文里的相关信息。由信息论，一个定宽状态无法无损保存无界上下文——越旧、越远的细节越会被压缩、覆盖。这正是 §1.3.1 将展开的“LLM 看起来记得、却没有持久事实”在数值层面的根。
...
而工业界已经把这条“非均匀分辨率”直接做进了架构。为了在百万 token 上还付得起算力，新一代模型不再对全程做均匀的全注意力：滑窗注意力是字面上的矩形窗，窗外直接置零；线性注意力与 SSM（如 MiniMax 的做法）是字面上的指数衰减核；KV 压缩（DeepSeek V4 的 CSA 与 HCA、Kimi 的 MLA）则是有损降采样——近端最近的 token 保留高分辨率、不压缩，越往远端、KV 被越激进地聚合：CSA 把每 m 个 token 压成一条，HCA 把每 m′（远大于 m）个 token 压成一条。换句话说，**所谓“1M context”不是一条处处等价的传送带，而是一组分辨率非均匀的滤波器：近端细、远端粗。**[^nonuniform-kv-ch1]

这里要专门强调一句，免得读者误以为“换个更强的闭源前沿模型就没事了”：**闭源前沿也不例外。** 第三方系统评测（如 Chroma 对 18 个前沿模型的 *Context Rot* 测试）显示，每一个模型都在每一个长度增量上掉点；前沿模型只是 rolloff 更缓，而非豁免。更说明问题的是，lost-in-the-middle 与干扰项这类退化，在大概率使用全注意力的模型上照样出现——说明它来自上面这些基本面地板，而不只是压缩作弊。Anthropic 自己的工程文档也直说，context rot 在远未触达容量上限时就已经开始。[^context-rot-ch1]

#### 1.1.2 时间轴：时间越旧，有效敏感度越低

长任务的失效看起来是“时间”问题——跑得越久越偏——但要拆成两半才讲得透：一半可约为空间问题，另一半不可约。

`记忆老化可约为空间衰减`。vanilla Transformer 除 KV cache 外没有跨步状态——“k 步前还记不记得”这件事，等价于“那个 token 还在不在窗口里、当前 token 还 attend 得到它吗”。随着生成推进，一个旧事实与当前光标的相对位置距离单调增大，于是按 §1.1.1 的四条空间机理被衰减，最终被窗口、压缩或逐出淘汰。所以“随时间遗忘”在机理上就是“随相对距离衰减”——时间问题塌缩成了空间问题。

`误差复合不可约`。即便给一个完美、无限、均匀的窗口（空间衰减为零），自回归仍然会随任务变长而劣化：每生成一个 token 都是一次采样，且它会成为后续所有 token 的条件；每步有一个小错误率 ε，长 T 步全对的概率约为 (1−ε) 的 T 次方——可靠性随 horizon 几何衰减。再叠加 exposure bias：训练时模型条件于正确前缀，推理时却条件于自己越来越偏离分布的输出。这一半是反馈回路自身的性质，**延长上下文救不了它**——只能靠降低单步 ε、缩短 horizon、独立验证、重采样。
...
这里给出一个后文会反复使用的可操作判据：**把丢失的信息重新塞回光标附近——如果失效被修复，它是空间问题，可由上下文工程补偿；如果模型本来就拿到了信息却仍然漂移、越错越多，它是不可约的时间核，只能由验证、缩短 horizon 和人在环上来兜住。**

### 1.2 容量上限不等于有效工作区间：理论预测，实测印证

§1.1 的机理预测了一个直接后果：同一个上下文窗口，越往远端越不可用。这一节把它落到事实，并给出工程上该怎样对待它。

“支持 1M token context”首先是能力上限，不是工程上可以无脑塞满的建议值。长上下文最容易误导团队的地方，在于大家把“能放进去”误当成“同样擅长用好”。事实层面已经很清楚：
...
- *Lost in the Middle* 说明相关信息在开头或结尾时更容易被利用，放在中间时表现会下降；*Found in the Middle* 进一步把这种现象概括为 `"U-shaped attention bias"`——这正是 §1.1 那条“有效分辨率从光标附近向远处递减”曲线在检索任务上的形状。[^ltm][^fitm]
- NoLiMa 和 LongCodeBench 又把问题推到更接近真实工作的场景：去掉字面匹配线索、或进入代码理解与修复任务后，长上下文能力会明显变脆——NoLiMa 在 32K 上就把 GPT-4o 从 99.3% 打到 69.7%。[^nolima][^longcodebench]
...
结论是：最大 context 是容量上限，最佳工作区间才是设计目标。Agent 系统不能默认“把更多材料塞进窗口”就会更可靠，而应当反过来设计任务：让模型看到更短、更密、更靠近当前决策的证据切片，把状态、历史、产物和验证证据移到运行时事实流里，再按需调回 prompt。
...
对 harness 特别有启发，因为一个长期运行的 agent 同样需要这两类机制：session 事实流、validator 通过的 artifact、role / scope / worktree 边界，是它的“锁存器”；validator、replay、dashboard、人工升级、角色重分配和恢复逻辑，是它的“负反馈环”。长任务 agent 最容易犯的错，恰好就是把这两类机制都省掉：既没有东西把关键事实锁住，又没有东西把错误输出重新送回系统做纠偏。结果就是短期看速度很快，长期看状态漂移、工具误用、上下文污染和错误自信一起累积。
...
§1.1 讲清了模型天然缺什么，§1.3 讲清了外层控制系统是什么、从哪来。把两者对起来，本书后续的每一项 harness 能力，就都可以被读成“对某一条失效机理的对症补偿”。下面这张表是全书的提纲：左边是模型层“天然缺什么”，右边是 harness 层“补什么、在哪一章展开”——此刻表里的术语（事实流、验证、正/负反馈、人在环上）都已在 §1.3 定义过。
