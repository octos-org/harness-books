...
[^ltm]: Nelson F. Liu et al., *Lost in the Middle: How Language Models Use Long Contexts.* 见第 19 章参考文献 11。
[^fitm]: Cheng-Yu Hsieh et al., *Found in the Middle: Calibrating Positional Attention Bias Improves Long Context Utilization.* 见第 19 章参考文献 12。
[^anthropic-long]: Anthropic, *Prompting best practices: Long context prompting.* 本节引用其关于长文档前置、query 后置与约 30% 质量改进的建议；见第 19 章参考文献 13。
[^gemini15]: Google, *Introducing Gemini 1.5, Google's next-generation AI model.* 本节引用其中关于 NIAH 命中率、1M context pricing tier 与长时延预期的表述；见第 19 章参考文献 14。
[^google-long-context]: Google AI for Developers, *Long context.* 本节引用其关于 performance variability、retrieval-cost tradeoff 与 caching 的说明；见第 19 章参考文献 15。
[^nolima]: Ali Modarressi et al., *NoLiMa: Long-Context Evaluation Beyond Literal Matching.* 本节引用其 32K 条件下模型退化统计与 GPT-4o 从 99.3% 降至 69.7% 的结果；见第 19 章参考文献 16。
[^longcodebench]: Stefano Rando et al., *LongCodeBench: Evaluating Coding LLMs at 1M Context Windows.* 本节引用其代码任务样例中 Claude 3.5 Sonnet 与 Qwen2.5 的退化数据；见第 19 章参考文献 17。
[^longllmlingua]: Huiqiang Jiang et al., *LongLLMLingua: Accelerating and Enhancing LLMs in Long Context Scenarios via Prompt Compression.* 本节引用其对长上下文三类问题的总结，以及 NaturalQuestions 与 LooGLE 的压缩收益结果；见第 19 章参考文献 18。
[^minference]: Huiqiang Jiang et al., *MInference: Accelerating Pre-filling for Long-Context LLMs via Dynamic Sparse Attention.* 本节引用其关于 8B 模型处理 1M tokens 需 30 分钟的摘要数字；见第 19 章参考文献 19。
[^openai-pricing]: OpenAI, *API Pricing*, accessed 2026-04-23. 本节引用 GPT-5.4 与 GPT-5.4 mini 的输入价格，以及 “under 270K” 的定价限定；见第 19 章参考文献 20。
...
第 1 层是 AI 编码过程的驾驭工程。这里 agent 是工程团队的生产工具，最终产品仍然是代码、服务、应用或基础设施。OpenAI Codex 团队的 1M LOC 案例、Fowler 的代码 agent harness、AGENTS.md + docs 的知识注入模式，都属于这一层。[^layer1-harness-ch1]

第 2 层是 AI agent 自身的 harness 工程。这里产品本身就是 agent，用户直接把任务交给它。系统要处理 session 恢复、sandbox 权限、工具调用审计、长任务重启、客户环境隔离和合规签字。Anthropic 的 session / harness / sandbox 三抽象、Milvus 的执行层、Neo4j 的记忆层，更接近这一层。[^layer2-harness-ch1]

Octos 同时踩在两层上：团队用 AI 写代码，也在构建 agent 平台。因此本书采用 Octos 的故障和里程碑作为主线，但会借用原书中两层共同适用的稳定原则：状态持久化、工具可问责、验证独立、知识可晋升。

[^layer1-harness-ch1]: 本段综合 Fowler 关于 coding agent harness 的框架，以及 OpenAI Codex 团队 1M LOC / `AGENTS.md + docs` 的工程实践；见第 19 章参考文献 1、2。
[^layer2-harness-ch1]: 本段综合 Anthropic 关于 managed agents 与 long-running harness 的接口抽象，以及 Milvus 的 execution layer、Neo4j 的 memory/context layer 视角；见第 19 章参考文献 3、4、6、7。
...
原书里最有冲击力的案例来自 OpenAI Codex 团队：从 2025 年 8 月第一次提交到 2026 年 2 月，约五个月时间，三位工程师驱动 Codex 产出约 1M LOC、1500+ 个已合并 PR，平均每位工程师每天 3.5 个 PR，并宣称人类不直接写应用代码。[^openai-factory-ch2] 这个故事的重点不是“模型终于可以替代工程师”，而是工程师的工作对象上移了。
...
原书对 `AGENTS.md` 的处理很值得 Octos 直接采用：它应该短、稳定、可常驻上下文，只放仓库定位、工具链版本、工作区拓扑、高频红线和指向 `docs/` 的目录。真正细节放在 `docs/` 里，按任务需要加载。[^agents-catalog-ch2]
...
[^openai-factory-ch2]: OpenAI, *Harness Engineering: Leveraging Codex in an Agent-First World.* 本节引用其关于 1M LOC、1500+ 已合并 PR、三位工程师与低人工写码比例的案例数字；见第 19 章参考文献 2。
[^agents-catalog-ch2]: 本节关于 `AGENTS.md` 作为短目录、细节下沉到 `docs/` 的做法，综合 OpenAI 的 Codex harness 文章与 Fowler 对 coding-agent harness 的工程建议；见第 19 章参考文献 1、2。
[^review-boundary-ch2]: 本段前半关于 OpenAI 极端自动化案例的边界来自第 19 章参考文献 2；后半对医疗、金融、隐私与客户数据路径的要求，是本书针对高合规场景的工程外推。
...
[^karpathy-bookkeeping-ch4]: Andrej Karpathy, *LLM Wiki.* 本节借其 autoresearch / wiki 的双文件与 bookkeeping 思路，说明 `session.md` + `session.jsonl` 这类事实流设计的价值；见第 19 章参考文献 9。
