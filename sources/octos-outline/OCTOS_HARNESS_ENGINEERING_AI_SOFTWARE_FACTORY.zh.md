...
如果把过去一年里最有代表性的几类系统放在一起看，无论是 Claude Code、Codex，还是 OpenClaw、Hermes 乃至 Octos，都在证明同一个硬事实：强模型加提示词技巧，可以在演示里显得很聪明；但一旦进入真实流量、长任务、页面重载、多会话和多工具并发，瓶颈就会从“回答质量”迅速转移到“控制系统质量”。
...
这些经验并不抽象。Claude Code 和 Codex 提供了比较成熟的正向样本，OpenClaw 和 Hermes 提供了更广义 agent product 的相邻样本，Octos 则提供了一个失败样本：后台任务进度漂移、`run_pipeline` / `deep-search` 状态不一致、会话污染、产物契约缺口、验证器不完整，以及操作员盲区。本书会反复对照这些系统，不是为了做产品测评，而是为了说明同一个模式会怎样在不同系统表面重复出现：瓶颈不是模型，而是控制系统。
...
这一版的写法有一个明确取舍：不把“大书”的章节原样搬进来，而是沿着当前提纲，把原书中关于两层驾驭工程、OpenAI 1M LOC 案例、AGENTS.md 分层、四支柱、长程循环、skills 晋升和“多脑多手”组织方式的论证，和 Claude Code、Codex、OpenClaw、Hermes 的实际系统形态重新消化。读者先获得一条稳定主线，再按需去 `/original/` 查看完整原书。

---

## 0. 全书地图与阅读指南

这份单文件书稿源自 Octos 提纲，但主分析线不以 Octos 为中心。它保留既有叙事主线，并把 ZIP 源书的前置说明、阅读指南、附录和参考文献折叠进来，让整个论证可以在一个地方读完。
...
4. 像 Octos 这样还在补控制平面的系统，当前遇到的多数事故，本质上都能还原为同一类问题：系统把“类似聊天的输出”误当成“运行时事实”。
...
核心弧线保持不变：第 1 节提出控制系统视角，第 2 节先用 Claude Code、Codex、OpenClaw、Hermes 拉出成熟实践光谱，第 3 节再把 Octos 这个失败样本转化为失败类别，第 4 节和第 7 节把这些失败转化为运行时架构，后续章节区分 harness 能解决什么，以及哪些仍然依赖模型质量、治理和操作纪律。
...
因此本书后续会在理论、成熟样本和 Octos 失败样本之间来回切换。失败样本展示控制平面在哪里断裂；成熟样本展示哪些机制已经被工程化；理论解释为什么如果不把 session、tool、verification 和 replay 当作一个系统，同样的断裂会反复出现。
...
Claude Code 和 Codex 站在这两层的分界线附近：它们既是编码生产工具，也体现出长任务 harness 产品的形态。OpenClaw 和 Hermes 更明显属于 Layer 2：产品本身就是会话、记忆、gateway、skills、cron 和 sub-agent 的控制面。Octos 在本文里只承担一个角色：失败样本。它有价值，不是因为它代表行业，而是因为它把同一种结构性问题暴露得很具体。
...
## 3. 通用失败类别：Octos 把它们暴露得很典型

如果说第 2 章回答的是成熟系统在建设什么，那么这一章回答的是：一旦这些机制缺位，系统会怎样坏。这里用 Octos 当样本，不是因为 Octos 比别人更特殊，而是因为它把问题暴露得足够具体。我们不再讨论理念，而是把差距拆成可命名、可归档、可修复的失败类别。
...
原书把 Layer 2 的稳定基础拆成四根支柱，这也解释了为什么像 Octos 这样的系统最后都得走向这一套设计。
...
小型 fleet 则是发布真实性的底线。Harness bug 常常是时序型、多表面型的：单元测试能过，端到端本地也能过，但在两台真实主机、两个 session、一次重载、一次重连后才暴露。这类系统的发布门禁应该优先覆盖这些真实路径。
...
到这里，整本书的主线也就闭合了：从长上下文和长任务的根因出发，经过 Claude Code/Codex 等成熟样本、OpenClaw/Hermes 的相邻对照、Octos 失败样本、运行时架构、组织扩展与治理边界，最后落回可执行的工程姿态。
...
保留全部十篇的理由很具体：第 3 节中的失败样本属于 Layer 2 failure，而产生这本合并书的过程又位于 Layer 1 谱系中。把二者放在一起读，能让控制系统视角保持诚实。
