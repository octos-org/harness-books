...
4. 像许多仍在补控制平面的系统一样，本文掌握的一组失败案例当前遇到的多数事故，本质上都能还原为同一类问题：系统把“类似聊天的输出”误当成“运行时事实”。
...
核心弧线保持不变：第 1 节提出控制系统视角，第 2 节先用 Claude Code、Codex、OpenClaw、Hermes 拉出成熟实践光谱，第 3 节再把一组真实失败案例转化为失败类别，第 4 节和第 7 节把这些失败转化为运行时架构，后续章节区分 harness 能解决什么，以及哪些仍然依赖模型质量、治理和操作纪律。
...
#### 1.1.2 从 thermostat 到 software agent：agent 理论里的最小闭环

Franklin 和 Graesser 对 autonomous agent 的定义，直到今天都非常适合拿来做起点：agent 不是一次性运行的程序，而是会 `"senses that environment and acts on it, over time"` 的系统；他们甚至故意举了一个有点刺耳的例子，说 thermostat 也满足这个定义。[^agent-theory-ch1]
...
Wooldridge / Jennings 那条经典线进一步把 agent 的弱定义压成几项可工程化特征：自主性、反应性、主动性和社会性。[^agent-theory-ch1] 把这些概念翻成工程语言，一个 agent 至少包含下面这个循环：
...
#### 1.1.4 从 delay line 到 cache：为什么系统必须管理工作集
...
#### 1.1.5 从 Harold Black 到半导体反馈：为什么稳定要靠限幅与锁存
...
#### 1.1.6 回到现代 agent：控制论如何变成 harness engineering
...
保留全部十篇的理由很具体：第 3 节中的失败样本属于 Layer 2 failure，而这本书本身的写作与重构过程又位于 Layer 1 谱系中。把二者放在一起读，能让控制系统视角保持诚实。
