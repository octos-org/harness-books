...
这份单文件书稿源自一份内部提纲，但主分析线并不围绕某个具体产品品牌。它保留既有叙事主线，并把 ZIP 源书的前置说明、阅读指南、附录和参考文献折叠进来，让整个论证可以在一个地方读完。
...
下一章不再继续看成熟样本，而改用一组真实失败案例来回答另一个问题：当这些机制缺位时，系统具体会怎么坏。
...
如果说第 2 章回答的是成熟系统在建设什么，那么这一章回答的是：一旦这些机制缺位，系统会怎样坏。这里使用一组内部系统的真实失败案例，不点名，因为我们关心的是结构，不是品牌。与 Codex 把 `Thread` / `Turn` / `Item` 做成显式协议、Claude Code 把 transcript、resume、worktree 和多 agent 协调做成独立模块相对照，这组失败案例恰好暴露了另一面：事实流、回放、验证和调度一旦没有被系统化，系统就会重新滑回“看起来像聊天，其实不是运行时”的状态。我们不再讨论理念，而是把差距拆成可命名、可归档、可修复的失败类别。
...
合在一起，这些失败不是六个独立 bug。它们是同一种模式在不同表面上的表现：系统把类似聊天的输出当成了运行时事实。下一章会把这个模式转化为关于状态、事件、验证和回放的具体架构。
...
Claude Code 里的 `agentSummary.ts` 和 `diskOutput.ts` 给了一个很好的实现提醒：操作员视角不该只盯计数器，还要能快速抓到“这个后台 agent 最近在干什么”“它产出了什么文件”“日志在什么位置”。Codex 那边虽然更偏协议层，但 `Thread` / `Turn` / `Item` 的拆分也天然更适合喂给 dashboard，因为它把历史、执行和副作用分成了更清楚的观测面。
