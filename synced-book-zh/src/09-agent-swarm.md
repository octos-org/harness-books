...
![1M 与 5×200K 的量级对比](./img/context-economics.jpg)

*图：同样的总 token，左边一个 1M 的稠密上下文（发烫的致密块）和右边五个 200K 的局部分片（轻盈的小块）相比，前者的 attention 计算量级要重得多——这就是为什么账单、时延、可调度性三本账，往往都偏向“拆成多个小窗口”。*
