...
> **如果你没有机器学习背景，这一章其实只想让你记住关于这台机器的五件事**——后文所有结论都建立在它们之上。读到公式或细节卡住，尽管跳；但这五条请带走：
>
> - **它不读字，读“词元”。** 文本先被切成一个个词或子词，每个词元再变成高维空间里的一个坐标点（嵌入）；从此模型摆弄的全是坐标，不再是文字。
> - **每个词元带着一条“定宽”的信息管道往前走。** 这条贯穿所有层的“残差流”，宽度是固定的——它装得下的东西有上限，是后面一切容量与遗忘问题的总根源。
> - **它靠“注意力”回看上文，而注意力是一份要摊薄的守恒资源。** 要看的地方越多，分到每一处的就越少。
> - **算过的历史会被缓存起来复用（键值缓存）。** 但缓存住的是“算到哪儿了”，不是“想明白了什么”。
> - **归根到底，它是一台会不断续写自己输出的概率生成器，不是数据库、也不是状态机。** 它没有一个可靠的地方“存事实”；所谓“它记得”，往往只是那句话还留在窗口里而已。
>
> 把这五条揣进兜里，下一章“它为什么必然失效”就会顺理成章，而不是天降结论。
